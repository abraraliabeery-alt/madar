/*
0556687131
  سكربت مبسّط ومنظّم لصفحة إنشاء العقار (Facility Product Create)
...
(function(){
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    // =========================================================
    (function(){
      const btnStart=document.getElementById('voice-start'); const btnStop=document.getElementById('voice-stop'); const status=document.getElementById('voice-status'); const transcript=document.getElementById('voice-transcript'); const btnAnalyze=document.getElementById('voice-analyze'); const btnClear=document.getElementById('voice-clear'); const btnUndo=document.getElementById('voice-undo'); const documentInput=document.getElementById('ai-property-document'); const documentAnalyze=document.getElementById('ai-analyze-document');
      ...
      btnAnalyze&&btnAnalyze.addEventListener('click', async()=>{ const txt=transcript.value||''; if(!txt.trim()) return; const st=document.getElementById('voice-analyze-status'); if(st) st.textContent=t('analyzing','Analyzing...'); try { const out=await analyze(txt); const labels={title:'العنوان',type:'النوع',city:'المدينة',neighborhood:'الحي',price:'السعر',area:'المساحة',rooms:'الغرف',bathrooms:'الحمامات',floor_number:'الدور',parking_spaces:'المواقف',address:'العنوان'}; const summary=Object.keys(labels).filter(k=>out[k]!=null&&out[k]!=='').map(k=>`${labels[k]}: ${out[k]}`).join('\n'); const confidence=out.confidence!=null?`\n\nدرجة الثقة: ${Math.round(Number(out.confidence)*100)}%`:''; if(summary&&window.confirm(`راجع البيانات المستخرجة قبل تعبئة النموذج:\n\n${summary}${confidence}\n\nهل تريد تعبئتها؟`)) await fill(out); } catch(e) { console.error('Voice analyze error', e); } if(st) st.textContent=''; });
      documentAnalyze&&documentAnalyze.addEventListener('click',async()=>{ const file=documentInput?.files?.[0]; if(!file) return toast('اختر صورة المستند أولاً','error'); const original=documentAnalyze.textContent; documentAnalyze.disabled=true; documentAnalyze.textContent='جارٍ قراءة الصورة...'; try{ const body=new FormData(); body.append('document',file); const token=document.querySelector('meta[name="csrf-token"]')?.content||''; const response=await fetch(cfg.endpoints?.productDocumentAnalyze||'/voice/products/analyze-document',{method:'POST',headers:{'Accept':'application/json','X-CSRF-TOKEN':token},body}); const json=await response.json(); if(!response.ok||!json.data) throw new Error(json.message||'تعذر قراءة الصورة'); const out=json.data; const labels={document_type:'نوع المستند',title:'العنوان',city:'المدينة',neighborhood:'الحي',address:'العنوان',area:'المساحة',price:'السعر',plot_number:'رقم القطعة',plan_number:'رقم المخطط'}; const summary=Object.keys(labels).filter(k=>out[k]!=null&&out[k]!=='').map(k=>`${labels[k]}: ${out[k]}`).join('\n'); const warnings=Array.isArray(out.warnings)&&out.warnings.length?`\n\nتنبيهات:\n- ${out.warnings.join('\n- ')}`:''; if(summary&&window.confirm(`راجع البيانات المقروءة:\n\n${summary}${warnings}\n\nهل تريد تعبئة الحقول المتوافقة؟`)) await fill(out); }catch(error){ toast(error.message||'تعذر قراءة الصورة','error'); }finally{ documentAnalyze.disabled=false; documentAnalyze.textContent=original; } });
