...
class MarketingProductRequestController extends Controller
{
    ...
    {
        return view('public.marketing-product-requests.form');
    }
    ...
    public function store(Request $request)
    {
        ...
        $requestRecord = MarketingProductRequest::create([
            'name' => $data['name'],
            'phone' => $data['phone'],
        ...
        return response()->json([
            ...
            'matches' => $matches,
            'message' => 'تم استلام طلب التسويق بنجاح، وسنتواصل معك قريباً.',
        ]);
    }
    ...
    private function extractWithAi(string $description): array
    {
        ...
        try {
            ...
            \Log::warning('Marketing product request AI extraction failed: '.$e->getMessage());
            return $this->fallbackExtract($description);
        }
    }
    ...
    {
        return 'You extract marketing-product-request details from Arabic or English user text. The user wants to market/promote their product (real estate or other). Return only a JSON object with these keys and values in Arabic where relevant: product_type (product/category name string), city (string), neighborhood (string), price (number or null), rooms (integer or null), bathrooms (integer or null), area (number or null), purpose ("sell" / "rent" / "market" / null), marketing_channels (array of strings or null), target_audience (string or null), notes (string). If a value is missing, use null. Keep the output strictly as JSON.';
    }
    ...
    {
        $type = $extracted['product_type'] ?? null;
        ...
        if ($price !== null) {
            $query->whereHas('activeOffers', function ($q) use ($price) {
                $q->where('price', '>=', $price * 0.8);
            });
        }
