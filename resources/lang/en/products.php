<?php

return [
    'page_titles' => [
        'products' => 'Projects',
        'product_details' => 'Project Details',
    ],
    
    'headers' => [
        'products' => 'Projects',
        'browse_products' => 'Browse all available projects',
        'product_details' => 'Project Details',
        'product_information' => 'Project Information',
        'product_features' => 'Features',
        'product_attributes' => 'Attributes',
        'product_status' => 'Project Status',
        'product_comments' => 'Comments',
        'publisher_information' => 'Publisher Information',
        'actions' => 'Actions',
        'product_details_section' => 'Product Details',
        'share_product' => 'Share Product',
    ],
    
    'filters' => [
        'search' => 'Search',
        'search_placeholder' => 'Search in projects...',
        'category' => 'Category',
        'all_categories' => 'All Categories',
        'status' => 'Status',
        'all_statuses' => 'All Statuses',
        'active' => 'Available',
        'inactive' => 'Unavailable',
        'featured' => 'Featured',
        'verified' => 'Verified',
        'apply_filters' => 'Apply Filters',
        'reset' => 'Reset',
        'sort_by' => 'Sort by:',
        'latest' => 'Latest',
        'oldest' => 'Oldest',
        'name' => 'Name',
        'price' => 'Price',
    ],
    
    'status_badges' => [
        'featured' => 'Featured',
        'verified' => 'Verified',
        'available' => 'Available',
        'unavailable' => 'Unavailable',
    ],
    
    'product_info' => [
        'description' => 'Description',
        'features' => 'Features',
        'attributes' => 'Attributes',
        'category' => 'Category',
        'facility' => 'Facility',
        'publish_date' => 'Publish Date',
        'last_update' => 'Last Update',
        'undefined' => 'Undefined',
        'view_all_comments' => 'View all comments',
    ],
    
    'actions' => [
        'view_details' => 'View Details',
        'price_on_request' => 'Price on Request',
        'add_to_favorites' => 'Add to Favorites',
        'contact_publisher' => 'Contact Publisher',
        'book_now' => 'Book Now',
        'login_to_interact' => 'Login to interact with the product',
        'login' => 'Login',
        'add_to_cart' => 'Add to Cart',
        'buy_now' => 'Buy Now',
        'select_option' => 'Select Option',
        'select_rental' => 'Select Rental',
        'contact_now' => 'Contact Now',
    ],
    
    'social_media' => [
        'facebook' => 'Facebook',
        'twitter' => 'Twitter',
        'whatsapp' => 'WhatsApp',
    ],
    
    'empty_state' => [
        'no_products' => 'No projects found',
        'no_products_message' => 'No projects match the search criteria.',
        'add_new_product' => 'Add New Project',
    ],
    
    'pagination' => [
        'previous' => 'Previous',
        'next' => 'Next',
    ],
    
    'breadcrumb' => [
        'home' => 'Home',
        'products' => 'Projects',
        'product' => 'Project',
    ],
    
    'comments' => [
        'comments_count' => 'Comments',
        'view_all_comments' => 'View all comments',
    ],
    
    // Additional missing keys
    'show' => [
        'description' => 'Description',
        'no_description' => 'No description available',
        'no_attributes' => 'No attributes available',
        'bedrooms' => 'Bedrooms',
        'bathrooms' => 'Bathrooms',
        'square_meters' => 'Square Meters',
        'gallery' => 'Gallery',
        'attributes' => 'Attributes',
        'features' => 'Features',
        'comments' => 'Comments',
        'add_comment' => 'Add Comment',
        'comment_placeholder' => 'Write your comment here...',
        'rating' => 'Rating',
        'select_rating' => 'Select Rating',
        'submit_comment' => 'Submit Comment',
        'login_to_comment' => 'Login to add a comment',
        'login' => 'Login',
        'to_add_comment' => 'to add a comment',
        'no_comments' => 'No comments yet',
        'price' => 'Price',
        'book_now' => 'Book Now',
        'request_quote' => 'Request Quote',
        'remove_from_favorites' => 'Remove from Favorites',
        'add_to_favorites' => 'Add to Favorites',
        'property_details' => 'Property Details',
        'type' => 'Type',
        'floor' => 'Floor',
        'floors_count' => 'Floors Count',
        'parking_spaces' => 'Parking Spaces',
        'daily_rent' => 'Daily Rent',
        'monthly_rent' => 'Monthly Rent',
        'yearly_rent' => 'Yearly Rent',
        'deposit' => 'Deposit',
        'valid_until' => 'Valid Until',
        'view_all_offers' => 'View All Offers',
        'available_from' => 'Available From',
        'available_until' => 'Available Until',
        'facility' => 'Facility',
        'view_facility' => 'View Facility',
        'map_location' => 'Map Location',
        'open_in_google_maps' => 'Open in Google Maps',
        'contact_info' => 'Contact Information',
        'not_specified' => 'Not Specified',
        'all_attributes' => 'All Attributes',
        'pdf_preview' => 'PDF Preview',
        'sar' => 'SAR',
        'loading' => 'Loading...',
        'offer' => 'Offer',
        'sale_offer' => 'Sale Offer',
        'rent_daily' => 'Daily Rent Offer',
        'rent_monthly' => 'Monthly Rent Offer',
        'rent_yearly' => 'Yearly Rent Offer',
        'rent_offer' => 'Rent Offer',
        'recommended' => 'Recommended',
        'valid_until' => 'Valid until',
        'created_at' => 'Created at',
        'other_offers' => 'Other Available Offers',
        'all_offers' => 'All Available Offers',
        'error_loading_details' => 'An error occurred loading offer details',
        'error_loading_offers' => 'An error occurred loading offers',
        'unspecified' => 'Unspecified',
        'offer_details' => 'Offer Details',
        'book_visit' => 'Book a Visit',
        'no_offers' => 'No offers available',
        'sale_offers' => 'Sale Offers',
        'rent_offers' => 'Rent Offers',
        'login_required_book' => 'You must log in first to book an appointment. Do you want to go to the login page?',
        'login_required_quote' => 'You must log in first to request a quote. Do you want to go to the login page?',
        'quote_title' => 'Request a Quote',
        'full_name' => 'Full Name',
        'phone' => 'Phone Number',
        'email' => 'Email',
        'message' => 'Message',
        'message_placeholder' => 'Write your message here...',
        'cancel' => 'Cancel',
        'submit_quote' => 'Send Request',
        'sending' => 'Sending...',
        'quote_success' => 'Your quote request has been sent successfully! We will contact you soon.',
        'quote_error' => 'An error occurred while sending the request. Please try again.',
    ],

    'gallery' => [
        'title' => 'Photo Gallery',
        'close' => 'Close',
        'previous' => 'Previous',
        'next' => 'Next',
        'no_images' => 'No images to display.',
    ],

    'rent_periods' => [
        'daily' => 'Day',
        'monthly' => 'Month',
        'yearly' => 'Year',
    ],
    
    'search' => [
        'title' => 'Search Properties',
        'placeholder' => 'Search for properties...',
        'category' => 'Category',
        'all_categories' => 'All Categories',
        'city' => 'City',
        'all_cities' => 'All Cities',
        'property_type' => 'Property Type',
        'all_types' => 'All Types',
        'apartment' => 'Apartment',
        'villa' => 'Villa',
        'office' => 'Office',
        'commercial_shop' => 'Commercial Shop',
        'studio' => 'Studio',
        'price_range' => 'Price Range',
        'min_price' => 'Min Price',
        'max_price' => 'Max Price',
        'button' => 'Search',
        'clear_filters' => 'Clear Filters',
        'sort_by' => 'Sort by',
        'latest' => 'Latest',
        'price_low_to_high' => 'Price: Low to High',
        'price_high_to_low' => 'Price: High to Low',
        'name' => 'Name',
    ],
    
    'results' => [
        'title' => 'Search Results',
        'properties_available' => 'properties available',
        'no_results' => 'No Results Found',
        'no_results_message' => 'We could not find any properties matching your search criteria.',
        'view_all_properties' => 'View All Properties',
    ],
    
    'categories' => [
        'title' => 'Property Categories',
        'subtitle' => 'Browse properties by category',
        'properties_count' => 'properties',
    ],
    
    'by_category' => [
        'title' => ':category Properties',
        'subtitle' => 'Discover the best :category properties in your area',
        'properties_available' => 'properties available',
        'view_all_properties' => 'View All Properties',
        'no_properties' => 'No properties found',
        'no_properties_message' => 'No :category properties are currently available.',
        'other_categories' => 'Other Categories',
        'browse_other_categories' => 'Browse properties in other categories',
        'category_description' => 'About :category Properties',
    ],
    
    'property_card' => [
        'featured' => 'Featured',
        'has_offers' => 'Has Offers',
        'verified' => 'Verified',
        'location_unknown' => 'Location Unknown',
        'rooms' => 'rooms',
        'bathrooms' => 'bathrooms',
        'square_meters' => 'm²',
        'view_details' => 'View Details',
        'best_seller' => 'Best Seller',
        'limited_time' => 'Limited Time Offer',
        'in_stock' => 'In Stock',
        'available' => 'Available',
        'property_available' => 'Property Available',
        'offers_available' => 'Offers Available',
        'available_options' => 'Available Options',
        'purchase_options' => 'Purchase Options',
        'rental_options' => 'Rental Options',
        'recommended' => 'Recommended',
        'contact_for_pricing_description' => 'Contact us for the best prices and detailed information',
        'call_for_price' => 'Call for Price',
        'email_for_price' => 'Email for Price',
    ],
    
    'by_facility' => [
        'properties_available' => 'properties available',
        'view_facility' => 'View Facility',
        'view_all_properties' => 'View All Properties',
        'no_properties' => 'No properties found',
        'facility_description' => 'Facility Description',
    ],
    
    'hero' => [
        'title' => 'Find Your Perfect Property',
        'subtitle' => 'Discover the best properties in your area',
    ],
];
