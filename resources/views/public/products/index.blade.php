        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <form action="{{ route('public.products.index') }}" method="GET" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        <label for="price_range" class="block text-sm font-medium text-gray-700 mb-2">نطاق السعر</label>
                        <select name="price_range" id="price_range"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500">
                            <option value="">جميع الأسعار</option>
                            <option value="0-100000" {{ request('price_range') == '0-100000' ? 'selected' : '' }}>حتى 100,000 ريال</option>
                            <option value="100000-500000" {{ request('price_range') == '100000-500000' ? 'selected' : '' }}>100,000 - 500,000 ريال</option>
                            <option value="500000-1000000" {{ request('price_range') == '500000-1000000' ? 'selected' : '' }}>500,000 - 1,000,000 ريال</option>
                            <option value="1000000+" {{ request('price_range') == '1000000+' ? 'selected' : '' }}>أكثر من 1,000,000 ريال</option>
                        </select>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach($products as $product)
                    <div class="bg-white rounded-lg shadow-md overflow-hidden card-hover">
                        <div class="relative">
                            <img src="{{ $product->featured_image ?? 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80' }}"
                        ...
                        <div class="p-4">
                            ...
                            <p class="text-gray-600 text-sm mb-3">{{ $product->location ?? 'موقع غير محدد' }}</p>

                            <div class="flex items-center justify-between text-sm text-gray-500 mb-3">
                                <span><i class="fas fa-bed ml-1"></i>{{ $product->bedrooms ?? 0 }} غرف</span>
