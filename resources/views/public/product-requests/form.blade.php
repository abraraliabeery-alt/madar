...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50 dark:from-slate-900 dark:via-slate-950 dark:to-slate-900 py-12 px-4">
    <div class="max-w-4xl mx-auto">
        <!-- Hero -->
        <div class="text-center mb-10">
            <h1 class="text-3xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-3">
                ابحث عن عقارك بطريقة ذكية
            </h1>
            <p class="text-lg text-gray-600 dark:text-gray-300">
                اكتب أو تكلم بصوتك، وسنساعدك في إيجاد ما تبحث عنه فوراً.
            </p>
        </div>
        ...
        <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg rounded-3xl shadow-2xl border border-white/20 p-6 md:p-10">
            ...
            <div class="flex flex-wrap items-center gap-4 mb-6">
                <button id="voice-start" type="button" class="inline-flex items-center gap-2 bg-rose-600 hover:bg-rose-700 text-white font-semibold py-3 px-6 rounded-xl shadow transition">
            ...
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">وصف الطلب</label>
            <textarea id="description" rows="4" class="w-full p-4 rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" placeholder="مثال: أبغى شقة 3 غرف في شمال الرياض بحدود 800 ألف ريال..."></textarea>
            ...
            <div class="grid md:grid-cols-2 gap-6 mb-8">
                <div>
                    ...
                    <input id="name" type="text" required class="w-full p-3 rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">رقم الجوال *</label>
                    <input id="phone" type="tel" required class="w-full p-3 rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                </div>
            </div>

            <button id="submit-btn" type="button" class="w-full bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-bold py-4 rounded-2xl shadow-lg transition transform hover:-translate-y-1">
                ارسل الطلب
            </button>

            <!-- Results -->
            <div id="results" class="hidden mt-10">
                <div class="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 text-emerald-800 dark:text-emerald-200 mb-6" id="result-message"></div>
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-4">عقارات مطابقة</h3>
...
<script>
    const storeUrl = "{{ route('public.product-requests.store') }}";
    ...
    function renderExtracted(obj) {
        ...
        const labels = {
            ...
            budget: 'الميزانية', notes: 'ملاحظات'
        };
        for (const [k, v] of Object.entries(obj)) {
            if (k === 'notes' || v === null || v === '') continue;
            const chip = document.createElement('span');
            chip.className = 'inline-flex items-center gap-1 bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-200 px-3 py-1.5 rounded-full text-sm';
            chip.textContent = (labels[k] || k) + ': ' + v;
    ...
    submitBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            submitBtn.textContent = 'إرسال الطلب وعرض النتائج';
        }
    });
    ...
    function renderMatches(matches) {
        ...
        if (matches.length === 0) {
            matchesGrid.innerHTML = '<p class="col-span-full text-center text-gray-500 dark:text-gray-400">لا توجد نتائج مطابقة حالياً.</p>';
            return;
        }
        matches.forEach(m => {
            ...
            card.innerHTML = `
                ...
                <div class="p-4">
                    ...
                    <p class="text-emerald-600 font-semibold mb-1">${m.price || 'السعر عند الطلب'}</p>
