...
@section('title', __('facilities.by_category.title', ['category' => $category->name]))
...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-gradient-to-r from-primary-600 to-primary-800 text-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div class="text-center">
                <h1 class="text-4xl md:text-5xl font-bold mb-6">{{ __('facilities.by_category.title', ['category' => $category->name]) }}</h1>
                <p class="text-xl text-primary-100 max-w-3xl mx-auto">
                    {{ __('facilities.by_category.subtitle', ['category' => $category->name]) }}
                </p>
            </div>
        </div>
    </div>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4 space-x-reverse">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600">{{ $facilities->total() }} {{ __('facilities.by_category.facilities_available') }}</p>
                    </div>
                </div>
                <a href="{{ route('public.facilities.index') }}" class="text-primary-600 hover:text-primary-700 font-medium">
                    <i class="fas fa-arrow-right ml-2"></i>{{ __('facilities.by_category.view_all_facilities') }}
                </a>
            </div>
        </div>
    </div>
        ...
        @else
            ...
            <div class="text-center py-12">
                <div class="bg-white rounded-lg shadow-md p-8">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 mb-2">{{ __('facilities.by_category.no_facilities') }}</h3>
                    <p class="text-gray-600 mb-6">{{ __('facilities.by_category.no_facilities_message', ['category' => $category->name]) }}</p>
                    <a href="{{ route('public.facilities.index') }}" class="btn-primary text-white px-6 py-2 rounded-lg font-medium">
                        {{ __('facilities.by_category.view_all_facilities') }}
                    </a>
                </div>
            </div>
        @endif
        ...
        <div class="bg-white py-16">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center mb-12">
                    <h2 class="text-3xl font-bold text-gray-900 mb-4">{{ __('facilities.by_category.category_description', ['category' => $category->name]) }}</h2>
    ...
    <div class="bg-gray-50 py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                <h2 class="text-3xl font-bold text-gray-900 mb-4">{{ __('facilities.by_category.other_categories') }}</h2>
                <p class="text-lg text-gray-600">{{ __('facilities.by_category.browse_other_categories') }}</p>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                @foreach($categories ?? [] as $relatedCategory)
                    @if($relatedCategory->id !== $category->id)
                        <a href="{{ route('public.facilities.by-category', $relatedCategory) }}"
                           class="bg-white rounded-lg p-6 text-center hover:bg-primary-50 transition-colors shadow-md">
                            ...
                            <p class="text-gray-600 text-sm">{{ $relatedCategory->facilities_count ?? 0 }} {{ __('facilities.categories.facilities_count') }}</p>
                        </a>
                    @endif
                @endforeach
            </div>
...
function switchView(viewType) {
    ...
    localStorage.setItem('facilitiesByCategoryPreferredView', viewType);
}

// Set initial view based on user preference
document.addEventListener('DOMContentLoaded', function() {
    const preferredView = localStorage.getItem('facilitiesByCategoryPreferredView') || 'grid';
