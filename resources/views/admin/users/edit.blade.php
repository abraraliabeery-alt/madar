...
<div class="container-fluid">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center" data-intro="{{ __('admin.tour.users_edit_header_desc') }}" data-step="61">
                    ...
                    <div class="col-md-6">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('role_id') is-invalid @enderror" id="role_id" name="role_id" required>
                                        ...
                                        @foreach($roles as $role)
                                            <option value="{{ $role->id }}" data-role="{{ $role->name }}" {{ old('role_id', $user->roles->first()->id ?? '') == $role->id ? 'selected' : '' }}>
                                ...
                                <div class="mb-3" id="bank-section" style="display:none;">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('customer_banks') is-invalid @enderror" id="customer_banks" name="customer_banks[]" multiple>
                                        @php $selectedBanks = old('customer_banks', $user->customerBanks->pluck('id')->toArray()); @endphp
                                        @foreach($banks as $bank)
                                            <option value="{{ $bank->id }}" {{ in_array($bank->id, $selectedBanks) ? 'selected' : '' }}>
                                                {{ $bank->name }}
                                            </option>
                                        @endforeach
                                    </select>
