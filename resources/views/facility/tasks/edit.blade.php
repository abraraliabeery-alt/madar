...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold">تعديل مهمة #{{ $task->id }}</h1>
    </div>

    <form action="{{ route('facility.tasks.update', $task) }}" method="POST" class="bg-white p-6 rounded shadow max-w-3xl">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block mb-1">نوع المهمة (حر)</label>
                <input type="text" name="type" value="{{ old('type', $task->type) }}" class="w-full border rounded px-3 py-2" required>
            ...
            <div>
                ...
                <select name="status" class="w-full border rounded px-3 py-2" required>
                    @foreach(['open'=>'مفتوحة','assigned'=>'مُسندة','in_progress'=>'قيد التنفيذ','done'=>'منتهية','cancelled'=>'ملغاة'] as $v=>$label)
                        <option value="{{ $v }}" @selected(old('status',$task->status)===$v)>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="deadline" value="{{ old('deadline', optional($task->deadline)->format('Y-m-d')) }}" class="w-full border rounded px-3 py-2">
            ...
            <div class="md:col-span-2">
                ...
                <select name="assignees[]" multiple class="w-full border rounded px-3 py-2 min-h-[44px]">
                    @foreach($assignees as $u)
                        <option value="{{ $u->id }}" @selected(collect(old('assignees',$task->users->pluck('id')->all()))->contains($u->id))>{{ $u->name }}</option>
                    @endforeach
                </select>
